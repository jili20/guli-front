...
export default {
  data() {
    return {
      ...
      oneIndex: -1, // 做一级分类选中效果
      twoIndex: -1, // 做二级分类选中效果
      buyCountSort: '', // 按销量查询选中效果
      gmtCreateSort: '', // 按销量查询选中效果
      priceSort: ''
    }
  },
  created() {
  ...
  methods: {
    ...
    // 4 点击某个一级分类，查询对应二级分类(用绑定点击事件来查询二级分类）
    searchOne(subjectParentId, index) {
      ...
      // 点击某个一级分类进行条件查询，每次查询都调用这个方法，都是查询第1页，所以传1，传 page 也行，默认第1页
      this.gotoPage(1)
    ...
    // 6 根据销量排序
    searchBuyCount() {
      // 设置对应变量值，为了样式生效，销量排序有值有样式，其它排序为空没有样式，
      this.buyCountSort = '1'
      this.gmtCreateSort = ''
      this.priceSort = ''
